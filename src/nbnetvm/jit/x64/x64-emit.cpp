...
namespace jit{
	namespace x64{
		...
		*/
		...
		void x64_Emit_J_Relative32(uint8_t cc, uint32_t displ, uint8_t **bufPtr)
		{
			(*(*bufPtr)++) = 0x0F;
...
// 				szBit = 1;
		...
		{
			x64_Emit_J_Relative32(CC_ONLY(insn->getOpcode()), 0, codeBuf);
		}
...
/*			uint32_t caller_save_mask[] = X64_CALLER_SAVE_REGS_MASK;
		...
		void x64_Emitter::emit_insn(x64Instruction* insn)
		{
		      #ifdef _DEBUG_X64_BACKEND
			std::cout << "emitting instruction at " << (void*)current << " (IR " << (void*)insn << ") " << *insn << std::endl;
		      #endif
		...
		void x64_Emitter::emitBB(bb_t *bb)
		{
			...
			//	x64_Emit_JMP_Relative32(0, &current);
			//}
		}
	} //namespace x64
} //namespace jit
