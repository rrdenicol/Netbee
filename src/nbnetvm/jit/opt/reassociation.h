...
#ifdef _DEBUG_REASS
		std::cout << "Processo il nodo: " << std::endl;
...
#ifdef _DEBUG_REASS
				std::cout << "Voglio riscrivere il nodo con il registro " << *mi->second->getDefs().begin() << std::endl;
				std::cout << "Nodo prima di setDefReg è: " << std::endl;
				kid->printNode(std::cout, true);
				std::cout << std::endl;
#endif
				kid->setDefReg(*(mi->second->getDefs().begin()));
#ifdef _DEBUG_REASS
				std::cout << "Nodo dopo setDefReg è " << std::endl;
				kid->printNode(std::cout, true);
				std::cout << std::endl;
#endif
				assert(kid->getDefReg() == mi->second->getDefReg());
#ifdef _DEBUG_REASS
				std::cout << "Sostituzione è: " << std::endl;
				kid->printNode(std::cout, true);
				std::cout << std::endl;
#endif
...
#ifdef _DEBUG_REASS
				std::cout << "Sostituzione è(weirdo): " << std::endl;
				kid->printNode(std::cout, true);
				std::cout << std::endl;
#endif
...
#ifdef _DEBUG_REASS
		std::cout << "La definizione trovata dal grafo SSA per " << kid->getDefReg() << " è: " << std::endl;
		std::cout << "BB ID: " << place.first->getId() << std::endl;
...
void Reassociation<_CFG>::fix_stores(typename _CFG::BBType *bb /*, list<IRType *>::iterator ni */)
{
	...
	for(i = code.begin(); /* i != ni && */ i != code.end(); ++i) {
		...
		if( (*i)->getTree() && (*i)->getTree()->isStore() ){
			...
			std::cout << "definito dal nodo:" << std::endl;
...
//		++i;
		...
		if(si->second->getDefs().size() == 1) {
			...
			cout << '\n';
			std::cout << "Number of uses: " << si->second->getAllUses().size() << std::endl;
#endif
	...
	// Insert the weirdos (= constants, for now) at the head
	// ... but not before the phis.
